...
function Products() {
  ...
  const handleOnClick = (e, category:string) => {
    ...
    const filterProducts = getProducts().filter((element)=>{return element.category.toUpperCase() === category.toUpperCase() })
  ...
  const filterPrice = (e)=>{
    ...
    const filterProducts = getProducts().filter((element)=>{
      return (element.price > priceMenor && element.price< priceMayor)
    })
    setTotalProducts(filterProducts.length)
    setFilters(filterProducts)
  ...
  return (
    <>
      ...
      <div className='flex'>
        ...
        {/* SideBar */}
        <SideBar handleOnClick={handleOnClick} handleOnChange={handleOnChange} filterPrice={filterPrice} />
        ...
        <div className='w-4/5 py-16 px-20'>
          ...
          {/* Productos */}
          <div >
            <Cards max={totalCard} filter={filters} sort={order} currentPage={currentPage}  />
          </div>
          ...
          <div className='w-full h-7 flex justify-center mt-4'>
            ...
            <div className='w-44 '>
              <div className='flex justify-center text-2xl text-[#7B7B7B] font-bold'>
                ...
                <button onClick={handlePrevious} className='mr-6'><Image src={previous} /></button>
                ...
                <button className='flex justify-center w-8'>
                  <p>{(paginado+1===Math.ceil(totalProducts/totalCard))?'':paginado+2}</p>
                </button>

                <button onClick={handleNext} className='ml-6'><Image src={next} /></button>
              </div>
            </div>
