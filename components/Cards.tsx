...
import { getProducts, sortPriceMayorMenor, sortPriceMenorMayor, sortRelevance ,sortByName } from '../products'
  ...
  return (
    <div className={`grid ${numCols} `} >
      { products?.slice(currentPage,currentPage + max)?.map( (element)=>{
        return <Card key={element.id} id={element.id} name={element.name} price={element.price} img={element.image} />
      } )
       }
    </div>
  )
